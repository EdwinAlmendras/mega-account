...
def import_from_api(
    api_url: str = typer.Option("http://127.0.0.1:8000", "--api-url", "-u", help="API server URL"),
    ...
    """
    asyncio.run(_import_from_api(api_url, master_password))


async def _import_from_api(api_url: str, master_password: Optional[str]):
    ...
    async with AccountManager() as manager:
        try:
            ...
            typer.echo(f"\n✓ Successfully imported {len(accounts)} account(s)")
